...
require_env HEATWAVE_NL_SQL_MODEL_ID
...
envsubst ' ${BACKEND_PORT} ${DB_URL} ${DB_USER} ${DB_PASS} ${DB_POOL_SIZE} ${ADMIN_USER} ${ADMIN_PASS} ${CUSTOMER_USER} ${CUSTOMER_PASS} ${OCI_GENAI_API_KEY} ${OCI_GENAI_MODEL_ID} ${HEATWAVE_NL_SQL_MODEL_ID} ${K8S_NAMESPACE} ${BACKEND_IMAGE} ${FRONTEND_IMAGE} ${IMAGE_TAG} ${FRONTEND_PORT} ${BACKEND_URL} ${BACKEND_CUSTOMER_USER} ${BACKEND_CUSTOMER_PASS} ${BACKEND_ADMIN_USER} ${BACKEND_ADMIN_PASS}' \
